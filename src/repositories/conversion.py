...
class ConversionTaskRepository(BaseRepository[ConversionTask]):
    def __is_available(self, task: Optional[ConversionTask]) -> ConversionTask:
        if task is None:
            raise TaskNotFoundException(task_type=TaskType.CONVERSION)

        if task.is_deleted:
            raise TaskIsDeletedException(task_type=TaskType.CONVERSION, task_id=task.task_id)
    ...
    def get_unique_completed_tasks(self, db: Session, model_id: str) -> List[ConversionTask]:
        ...
        """
        conditions = [self.model.input_model_id == model_id, self.model.status == TaskStatus.COMPLETED]
